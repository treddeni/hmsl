...
function checkKey(e){if(e=e||window.event,e.keyCode==KEY_UP_ARROW){e.preventDefault();var n=getActiveElementInfo();n&&(n[0]--,$("#"+n[1]+n[0]).focus())}else if(e.keyCode==KEY_DOWN_ARROW){e.preventDefault();var n=getActiveElementInfo();n&&(n[0]++,$("#"+n[1]+n[0]).focus())}}function getActiveElementInfo(){var e=document.activeElement.id;if(e.indexOf("nodeInput")>-1)return[e.replace("nodeInput",""),"nodeInput"];for(var n=document.activeElement.className.split(" "),t=0;t<n.length;t++){var i=n[t];if(console.log(i),i.indexOf("fieldInput")>-1&&"fieldInput"!=i){var c=i.replace("fieldInput","");return[document.activeElement.id.replace(c,""),c]}}}var KEY_UP_ARROW=38,KEY_DOWN_ARROW=40,KEY_LEFT_ARROW=37,KEY_RIGHT_ARROW=39;
function generateHeaderMarkup(e){var t='<select id="projectSelector" class="form-control" onchange="selectProject()">';t+='<option id="selectProjectOption" value="0">Select Project...</option>';for(var n=0;n<e.length;n++)t+='<option value="'+e[n].id+'">'+e[n].name+"</option>";return t+='<option id="newProjectOption" value="-1">Create New Project...</option></select>',t+='<input id="editNewProjectNameInput" class="form-control"></input>',t+='<a href="#" class="btn btn-default" style="float:right" onclick="saveToDatabase()">Save to Database</a>'}function generateFieldsRowMarkup(){for(var e='<table id="fieldHeaderTable"><tr>',t=0;t<tree.fields.length;t++)e+=getFieldHeaderMarkup(tree.fields[t].name,t);return e+="</tr></table>"}function generateTreeMarkup(){for(var e='<table id="treeTable">',t=0;t<tree.nodes.length;t++)e+=addRow(tree.nodes[t],0,"",0);return e+="</table>"}function addRow(e,t,n,a){var d="",l="nodeRow depth"+t+" expanded parent"+a+" "+n,i=e.id,r='<div class="redips-drag pull-right"><i class="glyphicon glyphicon-move"></i></div>',o='<a href="#" class="pull-right btn btn-danger btn-xs" style="margin:2px 2px 0px 0px" onclick="deleteNode(this)"><i class="glyphicon glyphicon-remove"></i></a>',s='<a href="#" class="pull-right btn btn-info btn-xs" style="margin:2px 4px 0px 0px" onclick="copyNode(this)"><i class="glyphicon glyphicon-plus"></i></a>',p='<a id="expandID'+i+'" href="#" style="margin-left:'+10*t+'px" onclick="toggle(this)" class="btn btn-xs show-expand"><span id="icon'+i+'" class="glyphicon glyphicon-chevron-down iconp'+a+'"></span></a>',c='<a id="expandID'+i+'" href="#" style="margin-left:'+10*t+'px" onclick="toggle(this)" class="btn btn-xs hide-expand"><span id="icon'+i+'" class="glyphicon glyphicon-chevron-down iconp'+a+'"></span></a>';if(d+='<tr id="rowid'+i+'" class="'+l+'"><td class="redips-rowhandler cell"><div style="width:499px">',d+=o+s+r,d+=e.nodes&&e.nodes.length>0?p:c,d+='<input id="nodeInput'+e.id+'" class="node" type="text" value="'+e.name+'" oninput="updateNodeName(this)"/></div></td>',d+="</tr>",e.nodes&&e.nodes.length>0)for(var u=0;u<e.nodes.length;u++)d+=addRow(e.nodes[u],t+1,n+" ancestor"+i,i);return d}function generateDataMarkup(){for(var e='<table id="dataTable">',t=0;t<tree.nodes.length;t++)e+=addDataRow(tree.nodes[t]);return e+="</table>"}function addDataRow(e){for(var t="<tr>",n=0;n<tree.fields.length;n++)t+=e.values&&e.values[tree.fields[n].name]?getFieldCellMarkup(tree.fields[n].name,e.id,e.values[tree.fields[n].name]):getFieldCellMarkup(tree.fields[n].name,e.id);if(t+="</tr>",e.nodes&&e.nodes.length>0)for(var n=0;n<e.nodes.length;n++)t+=addDataRow(e.nodes[n]);return t}function addColumn(e){$("#headerRow").append(getFieldHeaderMarkup(e,tree.fields.length)),$(".nodeRow").each(function(t,n){$(n).append(getFieldCellMarkup(e,n.id.replace("rowid","")))})}function getFieldHeaderMarkup(e,t){return'<td class="fieldHeaderCell"><div style="width:80px"><input id="colHeaderInput'+e+'" class="fieldHeaderInput" type="text" value="'+e+'" style="width:50px"/></div></td>'}function getFieldCellMarkup(e,t,n){var a="";return n&&(a=n),'<td class="cell"><div style="width:80px;height:21px"><input class="fieldInput fieldInput'+e+'" type="text" id="'+e+t+'" value="'+a+'" oninput="updateFieldValue(\''+e+"', '"+t+"')\"/></div></td>"}function getAssemblyMarkup(e,t,n){var a=findNodeInTree(e),d=findParentInTree(e);return addRow(a,n,"ancestor"+d.id,d.id)}
function displayProject(){$("#fieldsRow").html(generateFieldsRowMarkup()),$("#fieldsRow").css("right",scrollBarWidth+"px"),$("#data").html(generateDataMarkup()),$("#redips-drag").html(generateTreeMarkup()),$("#redips-drag").css("bottom",scrollBarWidth+"px"),redips.init(),initProjectActions()}function projectActionEnterPressed(){if("addField"==$("#projectActionSelector").val()){var e=$("#projectActionInput").val();tree.fields.push({name:e}),addColumn(e),$("#projectActionInput").hide(),$("#projectActionSelector").val("blank"),refreshDataModelDisplay()}}function createNewProject(){$("#selectProjectOption").remove();var e=$("#editNewProjectNameInput").val();$("#editNewProjectNameInput").hide();var t=projects.nextProjectID.toString(),o=document.createElement("option");o.text=e,o.value=t,$(o).insertBefore("#projectSelector option:nth-child("+$("#projectSelector").length+")"),$("#projectSelector").val(t),$.ajax({type:"POST",url:"/api/addProject?projectName="+e}),tree={projectID:t,projectName:e,version:1,nextNodeID:2,fields:[],nodes:[{id:1,name:e,nodes:[]}]},displayProject(),saveToDatabase(),refreshDataModelDisplay()}var handleProjectAction=function(){"addField"==$("#projectActionSelector").val()&&($("#projectActionInput").val("Type New Field Name and Press Enter to Create"),$("#projectActionInput").show().focus().select())};
