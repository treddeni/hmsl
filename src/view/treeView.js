var treeView = 
{
  ...
  display: function()
  {
    ...
    function zoom() { svgGroup.attr("transform", "translate(" + d3.event.translate + ")scale(" + d3.event.scale + ")"); }   // Define the zoom function for the zoomable tree
    ...
    function initiateDrag(d, domNode) 
    {
      ...
      if (nodes.length > 1) 
      {
        ...
        nodePaths = svgGroup.selectAll("path.link").data(links, function(d) { return d.target.id; }).remove();
        
        // remove child nodes
        nodesExit = svgGroup.selectAll("g.node").data(nodes, function(d) { return d.id; }).filter(function(d, i) { return d.id != draggingNode.id; }).remove();
      }
  
      // remove parent link
      parentLink = tree.links(tree.nodes(draggingNode.parent));
      svgGroup.selectAll('path.link').filter(function(d, i) { return d.target.id == draggingNode.id; }).remove();
    ...
    dragListener = d3.behavior.drag().on("dragstart", dragstart).on("drag", drag).on("dragend", dragend);  // Define the drag listeners for drag/drop behaviour of nodes.
    ...
    function dragend(d)
    {
      ...
      if (selectedNode) 
      {
        ...
        project.removeNode(draggingNode);
        
        if(!selectedNode.children) { selectedNode.children = []; }
        selectedNode.children.push(draggingNode);
        
        project.expandNode(selectedNode);  // Make sure that the node being added to is expanded so user can see added node is correctly moved    
      } 
    ...
    {
      var data = [];
      if (draggingNode !== null && selectedNode !== null)   // have to flip the source coordinates since we did this for the existing connectors on the original tree
      {
        data = [{ source: { x: selectedNode.y0, y: selectedNode.x0 }, target: { x: draggingNode.y0, y: draggingNode.x0 } }];
      }
    ...
    //root.children.forEach(function(child) { project.fullyCollapseNode(child); });   // Collapse all children of roots children before rendering.
